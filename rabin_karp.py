...
def rabin_karp(text, pattern): #  Алгоритм Рабина-Карпа для поиска вхождений образца в текст
    result = []
    patternsum = sum(ord(s) for s in pattern)
    textwindowsum = sum(ord(text[j]) for j in range(len(pattern) * (len(text) >= len(pattern))))
    for i in range(len(text) - len(pattern) + (len(pattern) > 1)):
